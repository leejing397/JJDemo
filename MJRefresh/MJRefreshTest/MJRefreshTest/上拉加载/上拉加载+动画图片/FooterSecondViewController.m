...
@interface FooterSecondViewController ()<UITableViewDelegate,UITableViewDataSource>
@property (nonatomic,strong)NSMutableArray *footerArrayM;
@property (nonatomic,strong)UITableView *footerTabelView;
@end

@implementation FooterSecondViewController
...
- (UITableView *)footerTabelView {
    if (_footerTabelView == nil) {
        ...
        _footerTabelView.mj_footer = [ChibaoziFooter footerWithRefreshingBlock:^{
            NSLog(@"上拉加载中。。。");
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
